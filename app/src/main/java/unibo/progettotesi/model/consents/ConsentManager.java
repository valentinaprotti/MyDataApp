package unibo.progettotesi.model.consents;
...
public class ConsentManager {
     ...
     * @param service
     *            The GoBoBusService which is given consent.
    ...
    public static ServiceConsent askServiceConsent(IUser user, IService service) {
        ...
        if (!(service.getSecurityManager().verify(user.getSecurityManager().getPublicKey(), tokenFromUser,
                tokenSignedUser))) {
            throw new SecurityException("Encountered error during verify process - GoBoBusService side.");
        } // else

        byte[] tokenFromService = "Example Token From GoBoBusService".getBytes(), tokenSignedService = null;
        try {
            tokenSignedService = service.getSecurityManager().sign(tokenFromService);
        } catch (Exception e) {
            e.printStackTrace();
            throw new SecurityException("Encountered error during sign process - GoBoBusService side.");
        }
     ...
     * GoBoBusService to the specified User. If the current status is Active or
    ...
    public static void changeServiceConsentStatusForService(IUser user, IService service, ConsentStatus newStatus) {
        ...
        if (!changed) {
            ...
            if (newStatus != ConsentStatus.WITHDRAWN) {
                ...
                // già ritirato
                throw new IllegalStateException("Cannot change the status of a Withdrawn GoBoBusService Consent.");
            }
        }
     ...
     * Data types allowed are retrieved from GoBoBusService Registry, in order to
    ...
    public static OutputDataConsent askOutputDataConsent(IUser user, IService service) {
        ...
        if (consent == null)
            throw new IllegalStateException("Cannot issue Output Data Consent for service " + service.toString()
                    + " if GoBoBusService Consent is not Active.");
     ...
     *             the GoBoBusService Registry for this service.
     * @return
     */
